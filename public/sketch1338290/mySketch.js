let colors =
  "390099-9e0059-ff0054-ff5400-ffbd00-473BF0-f37748-d56062-fff-0c0a3e-7b1e7a-b33f62-f3c677"
    .split("-")
    .map((a) => `#${a}`);
...
function setup() {
  ...
  overallScale = min(windowWidth, windowHeight) / 800;
  // pixelDensity(2)
  ...
  for (var i = 0; i < width + 50; i++) {
    for (var o = 0; o < height + 50; o++) {
      overAllTexture.set(
        i,
        o,
        color(100, noise(i / 3, o / 3, (i * o) / 50) * random([0, 30, 60]))
      );
    }
  }
...
function draw() {
  ...
  webglCanvas.scale(overallScale);
  ...
  if (hand0) {
    ...
    webglCanvas.rotateY(
      -PI / 4 + frameCount / 200 + map(mouseX, 0, width, -PI, PI)
    );
    webglCanvas.rotateZ(
      frameCount / 300 + PI / 2 + map(mouseY, 0, height, -PI, PI)
    );
